...
1. 打开 `http://127.0.0.1:6866/env-config`，填写 `HOOK_URL / BASE_URL / API_KEY / MODEL`。
2. 打开各平台页面完成登录（扫码）。
3. 配置关键词、城市、薪资等参数后开始投递。

### 4. 健康检查

- 后端：`http://127.0.0.1:8888/api/health`
- 前端：`http://127.0.0.1:6866`

## 🔐 对外分享前的隐私隔离（强烈建议）

本项目会把你的敏感信息写入本地 SQLite 数据库（`db/getjobs.db`），包括：

- `config` 表中的 API Key / Webhook / Base URL / Model
- `cookie` 表中的各招聘平台登录态 Cookie
- 历史投递记录（如 `boss_data` / `liepin_data` / `zhilian_data`）

分享给朋友前，建议至少做到：

1. 不要直接打包你的 `db/getjobs.db`。
2. 删除运行日志与构建产物：`target/`、`build/`、`front/.next/`、`front/out/`、`*.log`。
3. 让朋友在自己的机器上重新配置 AI 参数与登录态。
...
- 🤖 AI配置
  > 当前版本推荐在页面 `http://127.0.0.1:6866/env-config` 中直接配置，配置会写入数据库 `db/getjobs.db`。  
  > 不需要手动创建根目录 `.env` 文件。下面变量说明仅用于解释字段含义。
